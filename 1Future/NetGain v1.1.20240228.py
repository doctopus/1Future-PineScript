...
// Input parameters
selected_option = input.string("NIFTY", "Trading Future:", options=["NIFTY", "ES", "ETH"], inline = "Trade")
isLong = input.string("Long", title="Position:", options=["Long", "Short"],inline = "Trade")

entry_price1 = input.float(defval = 0.0, title = "Entry Price", inline = "Price")
...
// Display the values
// label.new(x=bar_index, y=entry_price, text=str.tostring(entry_price) + "\nNum Sets: " + str.tostring(num_sets) + "\nSum Sets: " + str.tostring(sum_sets), style=label.style_label_down, color=color.blue)


// Style inputs for BuyPriceLine
labelSizeOption = input.string("Normal", title="Label Size", options=["Tiny", "Small", "Normal", "Large", "Huge"], group="Style", inline = "Label")
gainLineColor = input(color.new(color.green, 0), title="Gain Line Color", group="Style", inline = "Label")
lossLineColor = input(color.new(color.red, 0), title="Loss Line Color", group="Style", inline = "Label")

lineWidth = input.int(defval = 2, title="Line Width", minval=1, group="Style", inline = "Line")
//lossLineWidth = input.int(2, title="Loss Line Width", minval=1, group="Style", inline = "Line")
bars_behind_current = input(0, title="Length on Left", group="Style", inline = "Line")
